  ...
  def control
    loop do
      ...
      when /^(\?|？)(\s*|　*)(.*)/
        ...
        arr.push("Copyright c @flum_ 2015")
        arr.push("Please enter...")
        subwindow(arr, arr.size + 5)
      when /^(a|A|ａ|Ａ)(\s*|　*)(.*)/
        @q = "kill"
...
if ARGV[0] == nil
	puts "Window size is not specified."
	puts "Please specified windows height as first argument."
	exit
end
...
when /^regist$/
  oauth.oauth_first
  $flumtter = Flumtter.new(ARGV[0].to_i)
when /^(\d+)$/
  $flumtter = Flumtter.new(ARGV[0].to_i, $1)
else
  $flumtter = Flumtter.new(ARGV[0].to_i)
end
$flumtter.start
